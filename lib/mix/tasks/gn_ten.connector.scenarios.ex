defmodule Mix.Tasks.GnTen.Connector.Scenarios do
  @moduledoc "Runs provider-free gn-ten connector hardening scenarios."

  use Mix.Task

  alias StackLab.GnTen.ConnectorScenarios

  @shortdoc "Runs connector hardening scenarios"
  ...
  def run(args) do
    ...
    case ConnectorScenarios.validate_report(report) do
      :ok ->
  ...
  defp print_success(report, false) do
    Mix.shell().info("gn_ten.connector.scenarios passed")
  ...
  defp print_failure(failures, false) do
    Mix.shell().error("gn_ten.connector.scenarios failed")
