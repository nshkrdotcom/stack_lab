defmodule Mix.Tasks.GnTen.RestartFencing.Scenarios do
  @moduledoc "Runs provider-free gn-ten restart and fencing scenarios."

  use Mix.Task

  alias StackLab.GnTen.RestartFencingScenarios

  @shortdoc "Runs restart and fencing scenarios"
  ...
  def run(args) do
    ...
    case RestartFencingScenarios.validate_report(report) do
      :ok ->
  ...
  defp print_success(report, false) do
    Mix.shell().info("gn_ten.restart_fencing.scenarios passed")
  ...
  defp print_failure(failures, false) do
    Mix.shell().error("gn_ten.restart_fencing.scenarios failed")
