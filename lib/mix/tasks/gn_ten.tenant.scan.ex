defmodule Mix.Tasks.GnTen.Tenant.Scan do
  @moduledoc "Scans gn-ten tenant boundary posture."

  use Mix.Task

  alias StackLab.GnTen.TenantScanner

  @shortdoc "Scans tenant boundaries"
  ...
  defp run_single_root(opts) do
    ...
    case TenantScanner.scan(scan_opts) do
      {:ok, report} ->
  ...
  defp run_all_repos(opts) do
    case TenantScanner.scan_all_repos() do
      {:ok, report} ->
  ...
  defp print_success(report, false) do
    Mix.shell().info("gn_ten.tenant.scan passed")
  ...
  defp print_failure(report, false) do
    Mix.shell().error("gn_ten.tenant.scan failed")
    Mix.shell().error("root=#{report.root}")
    Mix.shell().error("mode=#{report.mode}")
    Mix.shell().error("checked_files=#{report.checked_files}")
    Enum.each(report.violations, &Mix.shell().error("  #{TenantScanner.format_violation(&1)}"))
  end
  ...
  defp print_all_success(report, false) do
    Mix.shell().info("gn_ten.tenant.scan --all-repos passed")
  ...
  defp print_all_failure(report, false) do
    Mix.shell().error("gn_ten.tenant.scan --all-repos failed")
