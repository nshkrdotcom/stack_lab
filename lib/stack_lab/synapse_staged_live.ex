defmodule StackLab.SynapseStagedLive do
  @moduledoc false

  @schema_version "stack_lab.synapse_staged_live.v1"
  @example_command ["stack_lab.proof_app.synapse.staged_live.v1"]
  ...
  def default_receipt_path do
    Path.expand("tmp/stack_lab/synapse_staged_live/receipt.json", File.cwd!())
  end
  ...
  def run(opts \\ []) do
    ...
    with :ok <- remove_stale_receipt(receipt_path),
      ...
      {output, status} when is_binary(output) ->
        {:error,
         error("synapse_staged_live_command_failed",
           exit_status: status,
      ...
      other ->
        {:error, error("synapse_staged_live_unexpected_result", returned: inspect(other))}
    end
  end
  ...
  defp read_receipt(path) do
    with {:ok, body} <- File.read(path),
      ...
      {:ok, decoded} ->
        {:error, error("synapse_staged_live_receipt_not_object", decoded: inspect(decoded))}

      {:error, %Jason.DecodeError{} = reason} ->
        {:error,
         error("synapse_staged_live_receipt_json_invalid", reason: Exception.message(reason))}

      {:error, reason} ->
        {:error, error("synapse_staged_live_receipt_read_failed", reason: inspect(reason))}
    end
  end
  ...
  defp validate_receipt(receipt) do
    with :ok <- require_equal(receipt["schema_version"], @schema_version, "bad_schema_version"),
         ...
         :ok <- require_equal(receipt["classification"], "staging_live", "bad_classification"),
         :ok <- require_equal(receipt["no_bypass"]["status"], "pass", "no_bypass_not_pass"),
         :ok <-
           require_equal(
             receipt["proofs"]["fixture_acceptance"]["status"],
             "pass",
             "fixture_acceptance_not_pass"
           ),
         :ok <- require_equal(receipt["proofs"]["run_start"]["status"], "pass", "run_not_pass"),
         :ok <-
           require_equal(
             receipt["proofs"]["run_start"]["feature_status"],
             "staging_live",
             "run_not_staging_live"
           ),
         :ok <-
           require_equal(
             receipt["proofs"]["governed_pipeline"]["status"],
             "pass",
             "pipeline_not_pass"
           ),
         :ok <-
           require_equal(receipt["proofs"]["timeline"]["status"], "pass", "timeline_not_pass"),
         :ok <-
           require_equal(
             receipt["proofs"]["denial_path"]["status"],
             "pass",
             "denial_not_pass"
           ) do
      require_equal(receipt["proofs"]["evidence_chain"]["status"], "pass", "evidence_not_pass")
    end
  end
  ...
  defp default_example_receipt_path do
    Path.expand("tmp/stack_lab/synapse_staged_live/example_receipt.json", File.cwd!())
  end
