defmodule StackLab.SynapseLiveSlice do
  @moduledoc false

  @schema_version "stack_lab.synapse_live_slice.v1"
  @example_command ["stack_lab.proof_app.synapse.live_slice"]
  ...
  def default_receipt_path do
    Path.expand("tmp/stack_lab/synapse_live_slice/receipt.json", File.cwd!())
  end
  ...
  def run(opts \\ []) do
    ...
    with :ok <- remove_stale_receipt(receipt_path),
      ...
      {output, status} when is_binary(output) ->
        {:error,
         error("synapse_live_slice_command_failed",
           exit_status: status,
      ...
      other ->
        {:error, error("synapse_live_slice_unexpected_result", returned: inspect(other))}
    end
  end
  ...
  defp read_receipt(path) do
    with {:ok, body} <- File.read(path),
      ...
      {:ok, decoded} ->
        {:error, error("synapse_live_slice_receipt_not_object", decoded: inspect(decoded))}

      {:error, %Jason.DecodeError{} = reason} ->
        {:error,
         error("synapse_live_slice_receipt_json_invalid", reason: Exception.message(reason))}

      {:error, reason} ->
        {:error, error("synapse_live_slice_receipt_read_failed", reason: inspect(reason))}
    end
  end
  ...
  defp validate_receipt(receipt) do
    with :ok <- require_equal(receipt["schema_version"], @schema_version, "bad_schema_version"),
         ...
         :ok <-
           require_equal(receipt["proofs"]["await"]["status"], "accepted", "await_not_accepted"),
         :ok <-
           require_equal(
             receipt["proofs"]["runtime_projection"]["status"],
             "pass",
             "runtime_projection_not_pass"
           ),
         :ok <-
           require_equal(
             receipt["proofs"]["evidence_receipt"]["status"],
             "pass",
             "evidence_receipt_not_pass"
           ) do
      require_equal(receipt["proofs"]["denial_path"]["status"], "pass", "denial_not_pass")
    end
  end
  ...
  defp default_example_receipt_path do
    Path.expand("tmp/stack_lab/synapse_live_slice/example_receipt.json", File.cwd!())
  end
