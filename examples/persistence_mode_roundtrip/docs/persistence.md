# StackLab Persistence Mode Roundtrip Persistence

## Scope

StackLab Persistence Mode Roundtrip owns deterministic persistence profile matrix receipt owner. This document is package-local and is the persistence contract for `examples/persistence_mode_roundtrip` in `stack_lab`.
...
Structured GroundPlane policy data only; no environment, network, Postgres, Temporal, object store, or provider dependency.
...
```bash
cd examples/persistence_mode_roundtrip && mix test; root mix ci
```
...
Valid durability claims require explicit profile selection, adapter capability, migration or substrate preflight, redacted evidence, focused tests, repo QC, and a pushed commit. :integration_postgres and :full_debug_tracked receipts require explicit durable opt-in tags and preflight facts.
...
No migrations; harness records migration/preflight refs as evidence only.
