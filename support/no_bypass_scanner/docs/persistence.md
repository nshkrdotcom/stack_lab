# StackLab Product No-Bypass Scanner Persistence

## Scope

StackLab Product No-Bypass Scanner owns product no-bypass receipt contract owner. This document is package-local and is the persistence contract for `support/no_bypass_scanner` in `stack_lab`.
...
Ref-only scanner receipts only.
...
```bash
cd support/no_bypass_scanner && mix test; root mix ci
```
...
Valid durability claims require explicit profile selection, adapter capability, migration or substrate preflight, redacted evidence, focused tests, repo QC, and a pushed commit. No durable store selected. Receipts may carry proof refs from release evidence.
